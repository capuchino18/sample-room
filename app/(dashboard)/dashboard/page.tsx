          ...
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-lg">
            ...
            <form onSubmit={handleSimpanTransaksi}>
              <div className="mb-4 relative">
                ...
                <input type="text" value={searchSampleTerm} onChange={(e) => { setSearchSampleTerm(e.target.value); setIsDropdownOpen(true); setSelectedSample(null); }} onFocus={() => setIsDropdownOpen(true)} placeholder="Ketik nama / seri sampel..." required={!selectedSample} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#E31B23]" />
                {isDropdownOpen && (
                  <ul className="absolute z-10 w-full mt-1 bg-white border border-slate-300 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                    {filteredSamples.length > 0 ? (
                      filteredSamples.map((s) => (
                        <li key={s.id} onMouseDown={() => { setSelectedSample(s); setSearchSampleTerm(`${s.brand} - ${s.name || s.nama_sampel || s.seri}`); setIsDropdownOpen(false); }} className="px-4 py-2 hover:bg-slate-100 cursor-pointer border-b border-slate-50 text-sm">
              ...
              <div className="mb-4">
                ...
                <input type="number" min="1" value={qty} onChange={(e) => setQty(e.target.value)} required placeholder="Masukkan angka..." className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#E31B23]" />
              </div>
              {transactionType === 'KELUAR' && (
                <div className="mb-4">
                  <label className="block text-sm font-semibold text-slate-700 mb-1">Diberikan Kepada (Sales)</label>
                  <select required value={selectedSalesId} onChange={(e) => setSelectedSalesId(e.target.value)} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#E31B23] bg-white">
              ...
              <div className="mb-6">
                ...
                <textarea rows={2} value={keterangan} onChange={(e) => setKeterangan(e.target.value)} placeholder="Opsional (misal: untuk proyek Hotel X)..." className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#E31B23]"></textarea>
              </div>
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                <button type="submit" disabled={isSaving} className={`px-5 py-2 text-white rounded-lg font-bold shadow-sm disabled:opacity-50 ${transactionType === 'MASUK' ? 'bg-[#FBB03B] text-slate-900 hover:bg-[#e09c31]' : 'bg-[#E31B23] hover:bg-[#c9141b]'}`}>{isSaving ? 'Menyimpan...' : 'Simpan Transaksi'}</button>
              </div>
            </form>
          </div>
        </div>
