  ...
  return (
    <div className="p-6 md:p-10 w-full max-w-7xl mx-auto relative">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <button onClick={() => setIsExportModalOpen(true)} className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-lg font-medium transition shadow-sm flex items-center gap-2">
          📥 Export ke Excel
        </button>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <input type="date" value={selectedDateFilter} onChange={handleDateFilterChange} className="px-4 py-2 border border-slate-300 rounded-lg text-sm" />
        </div>
        <div className="flex items-center gap-3">
            ...
            {loading ? <tr><td colSpan={6} className="py-12 text-center text-slate-500">Memuat...</td></tr> : 
             currentSlideData?.map((item) => (
              <tr key={item.id} className="border-b hover:bg-slate-50">
                ...
                <td className="py-4 px-4"><span className={`px-3 py-1 rounded-full text-xs font-bold ${item.tipe === 'MASUK' ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-700'}`}>{item.tipe}</span></td>
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-md">
            ...
            <form onSubmit={handleExportExcel}>
              <div className="mb-4">
                ...
                <select value={exportType} onChange={(e) => setExportType(e.target.value)} className="w-full px-4 py-2 border rounded-lg">
                  <option value="SEMUA">Semua Transaksi</option>
                  <option value="MASUK">Hanya Barang Masuk</option>
                  <option value="KELUAR">Hanya Barang Keluar</option>
                </select>
              </div>
              <div className="mb-4"><label className="block text-sm font-semibold">Dari</label><input type="date" value={exportStartDate} onChange={(e) => setExportStartDate(e.target.value)} required className="w-full px-4 py-2 border rounded-lg" /></div>
              <div className="mb-6"><label className="block text-sm font-semibold">Sampai</label><input type="date" value={exportEndDate} onChange={(e) => setExportEndDate(e.target.value)} required className="w-full px-4 py-2 border rounded-lg" /></div>
              <div className="flex justify-end gap-3"><button type="button" onClick={() => setIsExportModalOpen(false)} className="px-4 py-2 text-slate-600">Batal</button><button type="submit" className="px-5 py-2 bg-emerald-600 text-white rounded-lg font-bold">Download Excel</button></div>
            </form>
          </div>
        </div>
      )}
